export default {

    homeTitle:'Full-cycle Software Development Company | Start Matter',
    contactUsButtonName: 'CONTACT US',
    info1: 'web page did not scroll to contact form options',
    info2: 'web page did not scroll to "Send request" button',
    info3: 'Confirmation is not displayed',
    nameStringPrompt: 'What is your name?',
    name: 'Butler',
    emailStringPrompt: 'Best email address to contact you',
    email: 'Butler@mail.com',
    countryStringPrompt: 'Your Country',
    country: 'Butlerland',
    contactStringPrompt: 'Contact number if you can share',
    contact: '+71515151212',
    confirmMessage: 'We\'ve received your inquiry.',

    contact2: '+71515151212',
    contact3: '+71515151212',


    contact4: '+71515151212',
    contact5: '+71515151212'

};